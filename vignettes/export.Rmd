...
**Source:** Copied from `outputDir` (written by the PPS step in `runPps()`).
...
**Columns:** Summary of **overlap** (logical: whether an episode’s start is ≤ previous episode’s end for that person): **colName**, **overlap** (TRUE/FALSE), **n**, **total**, **pct**, plus metadata. Only persons with more than one episode are considered.

**Purpose:** Count of persons with overlapping inferred pregnancy intervals. Used for data quality (overlaps may indicate algorithm or data issues).
