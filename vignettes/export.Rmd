...
To run export alone (e.g. after a previous pipeline run), use `exportPregnancies()` with the same `outputFolder` and your chosen `exportDir`. The same CSVs are produced; `exportFolder` in `runPregnancyIdentifier()` corresponds to `exportDir` in `exportPregnancies()`.
