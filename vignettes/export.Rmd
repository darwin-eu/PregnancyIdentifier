...
**Columns:** **colName** (value `"overlap"` for all rows), **overlap** (FALSE, TRUE, or NA per row), **n** (count of records in that category), **total** (total episode records, same for all rows), **pct** (percentage of records in that category), plus **cdm_name**, **date_run**, **date_export**, **pkg_version**. One row per overlap category: FALSE (no temporal overlap with previous episode), TRUE (episode start ≤ previous episode end within person), NA (no previous episode for that person, e.g. first episode).
