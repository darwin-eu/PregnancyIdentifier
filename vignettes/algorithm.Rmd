...
5.  **runEsd** — Reads `hipps_episodes.rds`, pulls gestational timing concepts (GW/GR3m), infers start dates and precision, harmonizes end and outcome from HIP vs PPS, and writes `final_pregnancy_episodes.rds`.
...
| | **PPS** | Produces **pps_outcome**. |
| | **Merge** | Keeps both **hip_outcome_category** and **pps_outcome** on each row. Does *not* choose a single outcome. |
| | **ESD** | **Sets final outcome.** Chooses **final_outcome_category** by picking between **hip_outcome_category** and **pps_outcome** using the same rules as for the end date. See [ESD vignette](esd.html). |
...
- **final_outcome_category** — Outcome category for this episode (e.g. LB, SB, PREG). Set by ESD by **picking one of** **hip_outcome_category** or **pps_outcome** using the same priority rules as for the end date.
