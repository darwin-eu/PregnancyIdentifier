...
| `export/` | De-identified summary CSVs and ZIP archive (if export step is used). See the [Export vignette](export.html) for file names, columns, and analysis use. |
...
6.  **exportPregnancies** — (Optional) Reads `final_pregnancy_episodes.rds` and writes shareable CSVs and a ZIP to `exportDir`. See the [Export vignette](export.html).
