...
Main parameters:
- **outputFolder** — Directory for pipeline outputs (RDS artifacts, logs, `runStart.csv`). Created if missing.
- **exportFolder** — Directory for shareable CSVs. Defaults to `file.path(outputFolder, "export")`. Optional; omit to use the default.
- **startDate**, **endDate** — Study window (defaults: 1900-01-01 to today).
- **minCellCount** — Small-cell suppression in exported summaries (default 5).
- **conformToValidation** — `FALSE` (default), `TRUE`, or `"both"`; see step 5 and the Export vignette.
...
| `export/` | De-identified summary CSVs (default: `outputFolder/export`). See the [Export vignette](export.html) for file names, columns, and analysis use. |
...
6.  **exportPregnancies** — Reads `final_pregnancy_episodes.rds` and writes shareable CSVs to `exportFolder` (default `file.path(outputFolder, "export")`). See the [Export vignette](export.html).
