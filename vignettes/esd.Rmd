...
4. **Study period filter** — Keeps rows where inferred_episode_start is on or after `startDate` and inferred_episode_end is on or before `endDate` (episodes with missing dates are retained).

5. **Collapse overlapping episodes** — Within each person, overlapping inferred intervals are merged into a single episode (union of intervals; non-date fields taken from the episode with the latest end). Rows with NA start or end are not merged. This ensures no two episodes for the same person overlap in time.

6. **Write** — Saves the final data frame to **final_pregnancy_episodes.rds** in `outputDir`. If `debugMode = TRUE`, **esd.rds** is written after step 2 (episode-level start inference only).
