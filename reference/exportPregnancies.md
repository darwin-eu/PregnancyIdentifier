...
- logger:
  ...
  Optional `log4r` logger. When `NULL` (default) a logger is created
  that writes detailed per-step start/finish timing to the console and
  to `export_log.txt` in `exportFolder`, so a slow or stuck step is
  obvious.
